...
impl<DBO, ID> CrudRepository<DBO, ID> {
    pub async fn count(&self) -> ResultErr<u64> {
        self.dao
            .count()
            .await
            .map_err(|_err| Error::Failure(ErrorWithCode::new("00MONCO", 500, "count error")))
    }
}
    ...
    DBO: From<DATA> + Clone + Send,
{
    ...
    async fn insert(&self, entity: &DATA) -> ResultErr<String> {
        ...
        self.dao.insert(&dbo).await.map_err(|err| {
    ...
    async fn quick_search(
        &self,
        chaine: &str,
        paths: Vec<QuickSearchPath>,
    ) -> ResultErr<Paged<DATA>> {
        let total_records = self.count().await? as u32;
        let page_size = 10;
        let total_page = ((total_records as f64) / (page_size as f64)).ceil() as u32;

        self.dao
            .quick_search(chaine, paths)
            .await
            .map(|dbo_opt| {
                dbo_opt
                    .into_iter()
            ...
            .map_err(|err| {
                Error::Failure(ErrorWithCode::new("00MONQS", 500, err.to_string().as_str()))
            })
    }

    async fn count(&self) -> ResultErr<u64> {
        self.dao.count().await.map_err(|err| {
            Error::Failure(ErrorWithCode::new("00MONCO", 500, err.to_string().as_str()))
        })
    }
}
